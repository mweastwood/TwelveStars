---
category: "devotional"
default_order: 13
default_title: "Prayer of St. Francis"
title: "Oración de San Francisco"
subtitle: "Oración por la Paz"
source_name: "Directorio Franciscano"
source_url: "https://www.franciscanos.org/oracion/orarpaz.html"
has_amen: true
history_author: "Father Esther Bouquerel (1912)"
history_origin: "La Clochette (1912); popularized during World War I"
history_context: "A devotional prayer for peace and charity first published in 1912, popularly attributed to St. Francis of Assisi after being printed on holy cards."
history_description: "A devotional prayer for peace and charity first published in 1912, popularly attributed to St. Francis of Assisi after being printed on holy cards."
---
¡{Señor, haz de mí un instrumento de tu paz|st_francis_1}!
{Que allí donde haya odio, ponga yo amor|st_francis_2};
{donde haya ofensa, ponga yo perdón|st_francis_3};
{donde haya discordia, ponga yo unión|st_francis_4};
{donde haya error, ponga yo verdad|st_francis_5};
{donde haya duda, ponga yo fe|st_francis_6};
{donde haya desesperación, ponga yo esperanza|st_francis_7};
{donde haya tinieblas, ponga yo luz|st_francis_8};
{donde haya tristeza, ponga yo alegría|st_francis_9}.

¡{Oh, Maestro!, que no busque yo tanto|st_francis_10}
{ser consolado como consolar|st_francis_11};
{ser comprendido, como comprender|st_francis_12};
{ser amado, como amar|st_francis_13}.

{Porque dando es como se recibe|st_francis_14};
{olvidando, como se encuentra|st_francis_15};
{perdonando, como se es perdonado|st_francis_16};
{muriendo, como se resucita a la vida eterna|st_francis_17}.

