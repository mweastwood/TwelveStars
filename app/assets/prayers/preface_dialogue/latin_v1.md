---
category: liturgy
default_order: 105
default_title: Preface Dialogue
title: Praefatio
subtitle: ""
source_name: "Maranatha"
source_url: "https://www.maranatha.it/RitoMessa/missaetext.htm"
history_author: "Traditional"
history_origin: "Apostolic Tradition of Hippolytus (c. 215 AD) & Roman Liturgy"
history_context: "The ancient dialogue preceding the Eucharistic Prayer, calling the congregation to lift up their hearts in thanksgiving to the Lord."
---
Sacerdos: {Dominus vobiscum.|preface_dialogue_1}
Populus: {Et cum spiritu tuo.|preface_dialogue_2}

Sacerdos: {Sursum corda.|preface_dialogue_3}
Populus: {Habemus ad Dominum.|preface_dialogue_4}

Sacerdos: {Gratias agamus Domino Deo nostro.|preface_dialogue_5}
Populus: {Dignum et iustum est.|preface_dialogue_6}
