...
def sample_verse(volume: int = 1, target_book: str = None):
    ...
    if target_book:
        ...
        if target_book not in vol_books:
            print(f"Error: Book {target_book} is not in Volume {volume}")
            return
    ...
    if not usfm_files:
        print(f"Error: USFM file for {chosen_book} not found")
        return
    ...
    if not matched_page:
        ...
        fpath = os.path.join(raw_dir, f"vol{volume}_page_{start_p}.json")
    ...
    if matched_lines:
        ...
        print(f"  (See raw_ocr file vol{volume}_page_{matched_page}.json)")
    print("=" * 80)

def main():
    parser = argparse.ArgumentParser(description="Sample a random verse from a volume and show PDF reader page number & raw OCR output.")
