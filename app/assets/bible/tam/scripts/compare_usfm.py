...
def compute_word_similarity(t1, t2):
    w1 = clean_text(t1).split()
    w2 = clean_text(t2).split()
    if not w1 or not w2:
        return 0.0
    overlap = set(w1) & set(w2)
    return len(overlap) / max(len(w1), len(w2))

def compute_char_similarity(t1, t2):
    s1 = clean_text(t1)
    s2 = clean_text(t2)
    ...
    for ocr_ch, ocr_v_dict in ocr_data.items():
        ...
        for v, ocr_txt in ocr_v_dict.items():
            if v in unam_ch_data:
                ...
                w_sim = compute_word_similarity(unam_txt, ocr_txt)
                word_sims.append(w_sim)
                
                # Character-level similarity
                c_sim = compute_char_similarity(unam_txt, ocr_txt)
